...
def check_instances(asg, launching_activities):
    ...
    for instance in asg["Instances"]:
        ...
        if last_activity_time is None:
            logger.info(f"auto-recycling is not complete because last_activity_time could not be determined for {instance_id}")
            return False

        last_launching_activity_times.append(last_activity_time)

    if not _compare_start_times(last_launching_activity_times, 2):
        logger.info("auto-recycling is not complete because the instances were not launched within 2 minutes of each other")
        return False

    return True


def _compare_start_times(last_launching_activity_times, delta):
...
def _monitor_autorecycle(event):
    output = event
    ...
    if output["counter"] > 20:
        ...
        output["message_content"][
            "text"
        ] = "Autorecycling appears to be taking too long :scream: please investigate."
...
def _describe_asg(component):
    ...
    lookup = [
        asg for asg in groups["AutoScalingGroups"] if asg["AutoScalingGroupName"].startswith(f"{component}-asg")
    ]
    ...
    if lookup:
        ...
        if lookup[0]["Instances"]:
            ...
            logger.info(
                "We have found 0 instances in the ASG, maybe this is in-out recycling"
            )
    else:
...
def _describe_scaling_activities(asg_name):
    asg_client = boto3.client("autoscaling", "eu-west-2", config=config)
