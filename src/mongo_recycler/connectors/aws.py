...
def create_instance_name_filters(component):
    instances = [re.sub(r"_[abc]$", "", component) + az for az in ["_a", "_b", "_c"]]
    return [{"Name": "tag:Name", "Values": instances}]


def describe_mongodb_instances(reservations):
    def describe_ip_address_from_instance(instance):
        try:
            return [
...
def instance_state(instance_descriptions):
...
class AWS:
    ...
    def get_launch_template_image_ids(self):
    ...
    def get_mongo_db_instances(self):
        client = boto3.client("ec2", region_name=self.region_name)
    ...
    def recycle_instance(self, instance):
